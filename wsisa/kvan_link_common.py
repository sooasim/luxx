...
def append_payment_notification(
    ...
    *,
    ...
    """
    본사/대행사 HQ·agency 어드민의 미확인 결제 알림용 큐.
    web_form.py 의 PAYMENT_NOTIFICATIONS_PATH (= data/payment_notifications.json) 와 동일 포맷.
    """
    tid = (tx_id or "").strip()
    if not tid or int(amount or 0) <= 0:
        return
    try:
