...
def signal_crawler_wakeup() -> None:
    """
    U-PAY 크롤러(kvan_crawler.py)에 "즉시 다시 크롤링해 달라"는 신호를 남긴다.
...
def _scrape_transactions_and_store(driver: webdriver.Chrome) -> None:
    """
    U-PAY 결제/취소 거래내역 페이지(/transactions)의 테이블을 크롤링하여
...
def _sync_kvan_to_transactions() -> bool:
    """
    kvan_transactions 에 쌓인 U-PAY 거래내역을
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # 1) 최근 U-PAY 거래 200건만 사용 (raw_text로 세션 KEY 추출 → 대행사 구분)
            cur.execute(
                """
            ...
            for kr in krows:
                ...
                if tx:
                    tx_id = tx["id"]
                    # agency_id 가 비어 있고, 이번 U-PAY 에서 대행사를 알 수 있다면 채워 넣는다.
                ...
                message = f"U-PAY {tx_type or '거래'} 자동 연동 (MID={mid}, 승인번호={approval})"
                cur.execute(
                    """
        ...
        print(f"[WARN] U-PAY ↔ 내부 transactions 매핑/생성 중 오류: {e}")
...
def _scrape_payment_links_and_store(driver: webdriver.Chrome) -> None:
    """
    U-PAY 결제링크 관리 페이지(/payment-link)의 리스트를 크롤링하여
    kvan_links 테이블에 저장.
        ...
        with conn.cursor() as cur:
            ...
            for el in link_elements:
                try:
                    ...
                    # 제목: 상호명(가맹점) → 상품명 → 첫 줄 (U-PAY UI 기준)
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if row:
                ...
                new_tx_id = datetime.utcnow().strftime("%Y%m%d%H%M%S%f")[-18:]
                message = (
                    f"U-PAY 결제 승인 (세션ID={session_id}, 승인번호={approval_no}, 카드={card_number})"
                )
                cur.execute(
                    """
                    INSERT INTO transactions (
...
def _get_agency_id_for_session(session_id: str) -> str | None:
    ...
    session_id 는 (1) 우리 쪽 세션 id 또는 (2) U-PAY 링크의 KEY... (거래 내역 팝업에서 옴) 일 수 있음.
...
def _lookup_internal_session_id_for_kvan_key(kvan_key: str) -> str:
    """U-PAY KEY… 에 매칭되는 admin_state 세션의 내부 id (숫자 세션 등)."""
        ...
        for idx, (btn, sid_hint, title_hint, expired_hint, badge_texts, raw_text_hint) in enumerate(card_items, start=1):
            try:
                ...
                if is_expired:
                    ...
                    # 테이블에 실제 데이터 행이 없거나, 첫 행이 '없습니다' 문구면 거래없음으로 간주 (U-PAY 문구 차이·로딩 지연 대응)
...
def _store_kvan_link_for_session(session_id: str, link: str) -> None:
    """admin_state.json 의 해당 세션에 U-PAY 결제 링크를 매핑해서 저장.

    링크 형식: https://store.k-van.app/p/KEY...?sessionId=KEY...&type=KEYED
    KEY 부터 &type=KEYED 앞까지가 U-PAY 세션 ID이며, 크롤러가 /payment-link 를 스크래핑할 때
    ...
    try:
        ...
        print(f"[WARN] admin_state 에 U-PAY 링크 저장 실패: {e}")
...
def _fill_payment_link_form_and_get_url(
    ...
    try:
        ...
        if link_text:
            ...
            # (1) 링크 복사 아이콘(버튼)을 클릭해서 U-PAY 내부 복사 로직 실행
            try:
                ...
                while time.time() < end_copy and not clicked_copy:
                    try:
                        ...
                        driver.execute_script("arguments[0].click();", copy_btn)
                        clicked_copy = True
                        print("[INFO] U-PAY 화면의 '링크 복사' 아이콘 버튼 클릭.")
                        break
                    except TimeoutException:
                        time.sleep(0.12)

            except TimeoutException:
                print("[WARN] '링크 복사' 아이콘 버튼을 찾지 못했습니다. U-PAY 내부 복사는 생략합니다.")
...
try {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).then(() => {
      console.log('[U-PAY] navigator.clipboard.writeText 성공');
    }).catch(e => {
      console.log('[U-PAY] navigator.clipboard.writeText 실패', e);
    });
  } else {
    ...
    try {
      ...
      console.log('[U-PAY] document.execCommand(\"copy\") 성공');
    } catch (e) {
      console.log('[U-PAY] document.execCommand(\"copy\") 실패', e);
    }
    document.body.removeChild(ta);
  }
} catch (e) {
  console.log('[U-PAY] 클립보드 복사 전체 실패', e);
}
""",
                    link_text,
                )
...
def main() -> None:
    ...
    - 세션 ID 또는 기본 JSON 을 읽어 U-PAY 결제 링크를 생성한다.
    ...
    try:
        ...
        if link_url:
            status = "link_created"
            msg = "결제 링크가 생성되었습니다. 고객이 링크로 결제하면 U-PAY 크롤러가 상태를 반영합니다."
        ...
        # U-PAY 크롤러(kvan_crawler.py)와 DB 동기화를 기준으로 관리한다.
        save_result_to_excel(RESULT_EXCEL_PATH, row, status, msg)
        save_result_to_json(str(result_json_path), status, msg)
